...
const Calender = () => {
  ...
  const imageData3 = [
    { imagesUrl: "https://backend.rmmbr.me/api/v1/post/get-file/0010-Flux_Dev_A_majestic_digital_painting_of_Lord_Shiva_sitting_in__1.jpeg", date: "2025-02-28T16:57:40.553Z" },
  ...
  const years = response.dateWiseImages.map(item => new Date(item.date).getFullYear());
...
function formatDate(timestamp) {
  ...
  return date.getFullYear() + '-' + 
         ...
         String(date.getDate()).padStart(2, '0'); // Uses local timezone
}
  ...
  return (
    <MainCalendar
      ...
      tileContent={({ date, view }) => {
        ...
        const imageObj = imageData.find(item => formatDate(item.date) === formattedDate);
