...
function ChatBox({ user }) {
  ...
  const savedUserDetails = getLocalStorageData("user")
  const storedUser = JSON.parse(savedUserDetails)
  const ADMIN_ID = "681e3db3e4adeb7a1bfa84c2" // Same admin ID
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      // Request messages for admin conversation
      socket.emit("message-page", ADMIN_ID)
    })

    // Handle all messages for current conversation
    socket.on("message", (allMessages) => {
      console.log("📩 User received messages:", allMessages)
      setMessages(allMessages || [])
    })

    // Handle conversation updates
    socket.on("conversation", (conversations) => {
      console.log("📩 User received conversation updates:", conversations)
      // Find admin conversation and update messages
      if (conversations && Array.isArray(conversations)) {
        const adminConversation = conversations.find((conv) => conv.userDetails._id === ADMIN_ID)
        if (adminConversation) {
          setMessages(adminConversation.messages || [])
        }
      }
    })
  ...
  const sendMessage = async (e) => {
    ...
    if ((message.trim() || imageFile) && socketRef.current) {
      ...
      const newMessage = {
        text: message.trim(),
        sender: storedUser?._id,
        receiver: ADMIN_ID, // Always send to admin
        msgByUserId: storedUser?._id,
        imageUrl: imageUrl || null,
        videoUrl: null,
      }

      console.log("User sending message to admin:", newMessage)
  ...
  return (
    <div className="container my-4 d-flex justify-content-between flex-column h-100">
      ...
      <div className="card-footer p-3 w-100">
        {selectedImage && (
          <div className="d-flex align-items-center mb-2">
            <button className="btn btn-danger btn-sm me-2" onClick={removeSelectedImage}>
              <FiX size={18} />
            </button>
            <img
