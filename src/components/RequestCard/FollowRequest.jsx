...
import { confirmCommentRequests, confirmFollowRequests, confirmPostRequests } from '../../utils/familytreeApi';
import CommonModal from '../CommonModal/CommonModal';
import { toast } from "react-toastify";
const FollowRequest = ({ item, getApi }) => {
  ...
  const handleSubmit = async (confirmation) => {
    try {
      const sandData = {
        status: confirmation ? "accept" : "reject",
        requestId: item?._id,
      };
      const request = await confirmFollowRequests(sandData);
      await toast.success(`${request.message}`);
      setShowModal(false)
      getApi()
      setTimeout(() => {
